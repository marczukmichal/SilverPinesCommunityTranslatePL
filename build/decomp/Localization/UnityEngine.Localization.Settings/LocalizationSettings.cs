...
public class LocalizationSettings : ScriptableObject, IReset, IDisposable
{
	...
	public static Locale ProjectLocale
	{
		get
		{
			...
			{
				if (Instance.GetAvailableLocales() is IPreloadRequired preloadRequired && !preloadRequired.PreloadOperation.IsDone)
				{
	...
	public virtual AsyncOperationHandle<Locale> GetSelectedLocaleAsync()
	{
		if (!m_SelectedLocaleAsync.IsValid())
		{
			if (GetAvailableLocales() is IPreloadRequired preloadRequired && !preloadRequired.PreloadOperation.IsDone)
			{
