...
public class TMP_FontAssetUtilities
{
	...
	private static TMP_Character GetCharacterFromFontAsset_Internal(uint unicode, TMP_FontAsset sourceFontAsset, bool includeFallbacks, FontStyles fontStyle, FontWeight fontWeight, out bool isAlternativeTypeface)
	{
		...
		if (flag || fontWeight != FontWeight.Regular)
		{
			...
			if (sourceFontAsset.characterLookupTable.TryGetValue(key, out character))
			{
			...
			{
				if (tMP_FontAsset.characterLookupTable.TryGetValue(unicode, out character))
				{
					if (character.textAsset != null)
					{
						isAlternativeTypeface = true;
				...
				if ((atlasPopulationMode == AtlasPopulationMode.Dynamic || atlasPopulationMode == AtlasPopulationMode.DynamicOS) && tMP_FontAsset.TryAddCharacterInternal(unicode, out character))
				{
					isAlternativeTypeface = true;
					return character;
				}
		...
		if ((atlasPopulationMode == AtlasPopulationMode.Dynamic || atlasPopulationMode == AtlasPopulationMode.DynamicOS) && sourceFontAsset.TryAddCharacterInternal(unicode, out character))
		{
			return character;
		}
		if (includeFallbacks)
