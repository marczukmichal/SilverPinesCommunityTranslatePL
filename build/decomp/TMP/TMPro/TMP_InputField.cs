...
public class TMP_InputField : Selectable, IUpdateSelectedHandler, IEventSystemHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler, ISubmitHandler, ICancelHandler, ICanvasElement, ILayoutElement, IScrollHandler
{
	...
	public void MoveToEndOfLine(bool shift, bool ctrl)
	{
		...
		else
		{
			...
			stringSelectPositionInternal = stringPositionInternal;
			int num4 = (caretSelectPositionInternal = (caretPositionInternal = num));
		}
		UpdateLabel();
	}
	...
	public void MoveToStartOfLine(bool shift, bool ctrl)
	{
		...
		else
		{
			...
			stringSelectPositionInternal = stringPositionInternal;
			int num5 = (caretSelectPositionInternal = (caretPositionInternal = num));
		}
		UpdateLabel();
	}
	...
	public override void OnPointerDown(PointerEventData eventData)
	{
		...
		if (allowInput || !m_OnFocusSelectAll)
		{
			...
			else if (m_isRichTextEditingAllowed)
			{
				...
				case CaretPosition.Left:
				{
					int num3 = (stringPositionInternal = (stringSelectPositionInternal = m_TextComponent.textInfo.characterInfo[cursorIndexFromPosition].index));
					break;
				}
				case CaretPosition.Right:
				{
					int num3 = (stringPositionInternal = (stringSelectPositionInternal = m_TextComponent.textInfo.characterInfo[cursorIndexFromPosition].index + m_TextComponent.textInfo.characterInfo[cursorIndexFromPosition].stringLength));
					break;
				}
				}
			}
			else
			{
				...
				case CaretPosition.Left:
				{
					int num3 = (stringPositionInternal = (stringSelectPositionInternal = ((cursorIndexFromPosition == 0) ? m_TextComponent.textInfo.characterInfo[0].index : (m_TextComponent.textInfo.characterInfo[cursorIndexFromPosition - 1].index + m_TextComponent.textInfo.characterInfo[cursorIndexFromPosition - 1].stringLength))));
					break;
				}
				case CaretPosition.Right:
				{
			...
			{
				int num10 = TMP_TextUtilities.FindIntersectingWord(m_TextComponent, eventData.position, eventData.pressEventCamera);
				if (num10 != -1)
				{
					caretPositionInternal = m_TextComponent.textInfo.wordInfo[num10].firstCharacterIndex;
					caretSelectPositionInternal = m_TextComponent.textInfo.wordInfo[num10].lastCharacterIndex + 1;
	...
	private void MoveLeft(bool shift, bool ctrl)
	{
		...
		if (ctrl)
		{
			num5 = FindPrevWordBegin();
		}
		else if (m_isRichTextEditingAllowed)
		{
			num5 = ((stringSelectPositionInternal <= 0 || !char.IsLowSurrogate(text[stringSelectPositionInternal - 1])) ? (stringSelectPositionInternal - 1) : (stringSelectPositionInternal - 2));
		}
		else
		{
			num5 = ((caretSelectPositionInternal < 1) ? m_TextComponent.textInfo.characterInfo[0].index : m_TextComponent.textInfo.characterInfo[caretSelectPositionInternal - 1].index);
			if (num5 > 0 && m_TextComponent.textInfo.characterInfo[caretSelectPositionInternal - 1].character == '\n' && m_TextComponent.textInfo.characterInfo[caretSelectPositionInternal - 2].character == '\r')
			{
				num5 = m_TextComponent.textInfo.characterInfo[caretSelectPositionInternal - 2].index;
			}
		}
		if (shift)
		{
	...
	private void Backspace()
	{
		...
		if (m_isRichTextEditingAllowed)
		{
			if (stringPositionInternal > 0)
			{
				...
				int num3 = (stringSelectPositionInternal = (stringPositionInternal -= num));
				m_Text = text.Remove(stringPositionInternal, num);
				num3 = (caretSelectPositionInternal = --caretPositionInternal);
				m_HasTextBeenRemoved = true;
				UpdateTouchKeyboardFromEditChanges();
				SendOnValueChangedAndUpdateLabel();
			}
		...
		{
			int num5 = caretPositionInternal - 1;
			int num6 = m_TextComponent.textInfo.characterInfo[num5].stringLength;
			if (num5 > 0 && m_TextComponent.textInfo.characterInfo[num5].character == '\n' && m_TextComponent.textInfo.characterInfo[num5 - 1].character == '\r')
			{
				num6 += m_TextComponent.textInfo.characterInfo[num5 - 1].stringLength;
				num5--;
			}
			m_Text = text.Remove(m_TextComponent.textInfo.characterInfo[num5].index, num6);
			int num3 = (stringSelectPositionInternal = (stringPositionInternal = ((caretPositionInternal < 1) ? m_TextComponent.textInfo.characterInfo[0].index : m_TextComponent.textInfo.characterInfo[num5].index)));
			num3 = (caretSelectPositionInternal = (caretPositionInternal = num5));
		}
		m_HasTextBeenRemoved = true;
		UpdateTouchKeyboardFromEditChanges();
		SendOnValueChangedAndUpdateLabel();
	...
	private void GenerateCaret(VertexHelper vbo, Vector2 roundingOffset)
	{
		...
		float num6 = (float)m_CaretWidth * font.faceInfo.lineHeight * num5 * 0.05f;
		m_CursorVerts[0].position = new Vector3(zero.x, y, 0f);
		m_CursorVerts[1].position = new Vector3(zero.x, num4, 0f);
		m_CursorVerts[2].position = new Vector3(zero.x + num6, num4, 0f);
		m_CursorVerts[3].position = new Vector3(zero.x + num6, y, 0f);
